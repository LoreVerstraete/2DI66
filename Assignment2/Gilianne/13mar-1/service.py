...
class service:
    ...
    def servicetime(extension, meancash, meancard,customerInfoIndiv):
        '''
        ...
        ######## added code for extension 1!
        # For extension 1 an if statement is added, this makes sure when extension 1 is used and the customer is in 
        # queue 0 the service time is 25% longer assuming the new employee works at queue 0
        if extension == 1 and customerInfoIndiv[1][service.nrQueue]==0:
            if customerInfoIndiv[1][service.cashCard] == "cash":
                servicetime = random.exponential(meancash*1.25) #servicetime cash
            elif customerInfoIndiv[1][service.cashCard] == "card":
                servicetime = random.exponential(meancard*1.25) #servicetime card
            else:
                print("card/cash not defined well")
            
            customerInfoIndiv[1].append(servicetime)
            return customerInfoIndiv   
        # Else extension 1 is not used or extension 1 is used and the customer is not in the queue with the new employee
        else:
            if customerInfoIndiv[1][service.cashCard] == "cash":
