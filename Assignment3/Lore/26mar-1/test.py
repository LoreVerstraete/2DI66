        ...
        while t<T:
            ...
            if e.type == Event.ELEVATORSTOPS:
                for elev_i in range(nrElevators):
                    ...
                    if len(queueElevator[elev_i]) > 0 or len(queueFloor[f]) > 0: #TODO if queueElevator for destination of that floor > 0 instead of queueuElevator > 0
                        removeCustomers = []
                        ...
                        for customer_floor in queueFloor[f]:
                            print("custUp",customer_floor.directionUp )
                            print("elevUp",elevator[elev_i].directionUp)
                        ...
                        if len(removeCustomers) > 0 or len(addCustomers) > 0:
                            ...
                            if len(queueElevator[elev_i]) < elevator[elev_i].MAXPEOPLE:
                                ...
                                customerThatWantInTheElevator = len(addCustomers) #TODO make sure only people in right direction step in
                                while len(addCustomers) > 0 and len(queueElevator[elev_i]) < elevator[elev_i].MAXPEOPLE: #TODO make sure only people in right direction step in
