'''
This class implements a priority queue in which customers are first sorted according their natural ordering.
This natural ordering can be implemented using the __lt__ function in the customer class.

Internally, we use a heapq to represent the queue of customers. Basically, this is just a class
that has the exact same functionality as a FCFS queue, but the ordering is different.

If there is no 'natural ordering' of the elements, be sure to specify the __lt__ function to compare items.
