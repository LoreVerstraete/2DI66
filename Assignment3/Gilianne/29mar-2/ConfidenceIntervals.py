...
class ConfidenceIntervals: 
    ...
    #         print("no Entry: ", noEntry, "means", mean(noEntry))
    #         print(array(self.noEnteryLimitOfTheElevator))
    #         confidenceIntervals[i][0] = mean(noEntry) - 1.96*sqrt(var(noE)/self.nrRuns)
    #         confidenceIntervals[i][1] = mean(noEntry) + 1.96*sqrt(var(noEntry)/self.nrRuns)
    #         means[i] = mean(noEntry)
    #     return confidenceIntervals, means
    ...
    def __str__(self):
        ...
        strElevator = ""
        for i in range(self.nrElevators):
            print("elevator", str(i), str(round(cIProbabilityPeopleInElevator[0][i][0],4)))
            print("elevator", str(i), str(round(cIProbabilityPeopleInElevator[1][i],4)))
            strNewElevator = (("Elevator "+ str(i) + " [ " + str(round(cIProbabilityPeopleInElevator[0][i][0],4)) + " , " + str(round(cIProbabilityPeopleInElevator[1][i],4))+ " , " + str(round(cIProbabilityPeopleInElevator[0][i][1],4)) + " ]" + "\n"))
            strElevator = strElevator + strNewElevator
