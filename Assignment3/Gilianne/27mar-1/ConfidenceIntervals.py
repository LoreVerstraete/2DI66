...
from numpy import append, cumsum, mean, minimum, split, zeros, cov, arange, sqrt, reshape, var, where
...
def simGG1Lindley2(arrDist, servDist, n):
...
lam = 0.7
mu = 0.9
arrDist = Distribution(stats.expon(scale=1/lam))
servDist = Distribution(stats.expon(scale=1/mu))   
n = 10000000
t1 = time.time()
w = simGG1Lindley2(arrDist, servDist, n) 
t2 = time.time()
...
for _ in range(nrRuns) :
    sim = simGG1Lindley2(arrDist, servDist, n)
    sumW += sim
meanW = sumW / nrRuns
t2 = time.time()
print("Simulation time: %f seconds" % (t2 - t1))
...
def ciMultipleRunsParallel(M, N, k):
    ...
    resultsAllWs = Parallel(n_jobs = numCores)(delayed(simGG1Lindley2)(arrDist, servDist, n) for _ in range(M))
...
print('Batch Means:')
t1 = time.time()
ci3 = ciBatchMeans(1000, 10000, 200)
t2 = time.time()
print("Simulation time: %f seconds" % (t2 - t1))
print(ci3)
print(mean(ci3))
...
def ciRegenerative(N): 
    sim = simGG1Lindley2(arrDist, servDist, N)
    # Now we're going to split the simulation results vector every time we 
    # encounter an empty system (i.e. a waiting time of zero)
    ...
    cv = cov(Yi, Ni)[0,1]  # sample covariance is element at (0, 1) or (1, 0) 
