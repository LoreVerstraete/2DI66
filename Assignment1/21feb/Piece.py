...
class Piece: 
    ...
    def PawnMoves(board, boardcolor, color, name, row, column, columsMoves, a, strategy):
        ...
        if strategy == Piece.STRATEGY_PAWN2: #strategy when pawn can move 2 squares at once for the first move
            if color == 1: # only applicable for white
                if row == 3: # only applicable for initial position
                    if board[row+a][column] == 0 and board[row+2*a][column]==0:  #moving 2 down, same column when both unoccupied
            ...
            for column in range(b.SIZE):
                if boardcolor[row][column] == Player:
                    ...
                    if boardpiece[row][column] == 1:
                        allMoves.append(Piece.PawnMoves(board, boardcolor, Player, 1, row, column, columsMoves[row][column], a, strategy))
...
# print(x)
print(Piece.allMoves(b.board, b.boardpiece, b.boardcolor, b.columsMoves, 1, Piece.STRATEGY_RANDOM))
